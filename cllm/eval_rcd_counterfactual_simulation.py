...
def run(args):
    ...
    for idx, fault_data in enumerate(faults):
        ...
        for _attempt in range(2):   # initial attempt + 1 case-level retry for API failure
            try:
                raw = llm.invoke(
                    prompt, system=SYSTEM_COUNTERFACTUAL_RCD, case_idx=idx)
                break
            except EvalLLMAPIError as e:
                unknown_reason = "api_failure"
                log_failure(idx, "api_failure", prompt, SYSTEM_COUNTERFACTUAL_RCD,
                            "", str(e))
                print(f"[RCDCf] Case {idx}: API FAILURE (attempt {_attempt+1}/2): {e}")
                if _attempt == 0:
                    continue   # retry the case
                break
            except Exception as e:
                unknown_reason = "unknown_error"
                log_failure(idx, "unknown_error", prompt, SYSTEM_COUNTERFACTUAL_RCD,
                            raw, str(e))
        ...
        if agent_target == "unknown" and unknown_reason is None:
            ...
            if unknown_reason in ("api_failure", "parse_failure"):
                log_failure(idx, unknown_reason, prompt, SYSTEM_COUNTERFACTUAL_RCD,
                            raw)
                print(f"[RCDCf] Case {idx}: UNKNOWN reason={unknown_reason}"
    ...
    # ── Final summary ─────────────────────────────────────────────────────────
    write_summary(out_dir, eval_records, mode="counterfactual", label="RCD")
