...
def draw_and_save(bn, target, samples, nodes):
    generator = gum.BNDatabaseGenerator(bn) #BNDatabaseGenerator从一个现成的贝叶斯网络获得数据，它是一个类的对象
    generator.drawSamples(samples) #采样samples个样本

    #get_node_name返回是字串X0,X1这样的
    var_order = [u.get_node_name(node) for node in range(nodes)]
    generator.setVarOrder(var_order) #固定采样顺序，这里是固定0~N
    generator.toCSV(target) #输出
...
def generate_random_dag(n, max_degree):
    ...
    perm = np.random.permutation(n) #生成一个随机排列，比如[43697……]长度为n
    current_i = 1
    while current_i < n:
        # 每次生成节点的父母
