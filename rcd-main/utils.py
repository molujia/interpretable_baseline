...
# 虽然这里用的方法是获得子节点，但在图里，边都是双向的，所以子节点也是父节点，连通=父子，所以可以这么写
...
def top_k_rc(normal_df, anomalous_df, bins=None, mi=[],
    ...
    _run_pc = lambda alpha: run_pc(data, alpha, localized=localized, mi=processed_mi,
                                   labels=i_to_labels, verbose=False)  # 这里的data已经带上fnode了
    ...
    for i in np.arange(_alpha, ALPHA_LIMIT, ALPHA_STEP):
        ...
        if len(new_neigh) == 0: continue
            ...
            rc += [n for n, _ in sorted(zip(new_neigh, f_p_values), key=lambda x: x[1], reverse=False)]
            #_order_neighbors(new_neigh, f_p_values) #从小到大排序

        # rc是除f节点外的节点，如果alpha足够大，致使这些节点都成为了f节点的子节点，确实该break
        if len(rc) == min_nodes: 
            # print('触发break了')
            break
    
    # print(cg.mi) #不是local_discovery的话，这里的cg.mi都是none，所以我感到纳闷：mi是代码作者自己添加的参数
    # mi 会动态累积那些在检验中被判定为独立的节点，但是初始化是np.empty，因此最终结果会有none。
    # print(rc)
